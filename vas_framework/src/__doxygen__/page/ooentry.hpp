 ...
 * VAS Framework will load a class with a public static member function named main which called "Launcher
 * Class" when the program start.
 * 
 ...
 * ```
 * 
 * The arguments are passed as an object of std::vector so it can be benifit from the std::vector which have
 * lot more function to access the array.
 * 
 * After all of the work is done, the class vas::ClassLoader is used to load up the class and start the
 * program. The ClassLoader will then initialize the static bool member variable (can be constant) with
 * member function vas::ClassLoader::load() and start the program with the specified Launcher class.
 *  
 * Example:
 * ```cpp
 * // Launcher.hpp
