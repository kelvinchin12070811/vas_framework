 ...
 * 
 * TextTools proide tools that read and write to console with UTF-8 encoding. This class is also designed to be
 * protable. The macro #VAS_WINDOWS_MODE must not defined for non windows platform to work.
 * 	  
 * Since the windows use UTF-16 as unicode strings, the tools must convert all UTF-8 strings to UTF-16 before calling
 * all native IO function. This is the reason that the macro #VAS_WINDOWS_MODE must be defined on Windows platform.
 * However, this macro must not defined on other not Windows operating system since most of them support UTF-8
 * natively.
 */
 ...
 * overloading to provide std's console IO stream feature. The helper stream currently only support output (operator<<)
 * and input (operator>>). For other function, use the member function getWrapedStream() to get the native IO stream
 * on user's platform.
 * 
 ...
 * 
 * @note The user must create an instance of the helper class with rvalue. This is because it will only dump the stream
 * to the native stream when its' destructor get called.
 * 
